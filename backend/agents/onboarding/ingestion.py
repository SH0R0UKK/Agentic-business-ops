"""
File Ingestion Utilities for Onboarding
Processes PDFs, images (with OCR), and text files into structured format
"""

import os
import mimetypes
from pathlib import Path
from PIL import Image
from pypdf import PdfReader
import pytesseract
import base64
from io import BytesIO

# Tesseract Configuration (optional - OCR will be skipped if not installed)
TESSERACT_PATH = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
if os.path.exists(TESSERACT_PATH):
    pytesseract.pytesseract.tesseract_cmd = TESSERACT_PATH


def image_to_base64(image: Image.Image) -> str:
    """Convert PIL Image to base64 string for LLM processing."""
    buffered = BytesIO()
    image.save(buffered, format="PNG")
    return base64.b64encode(buffered.getvalue()).decode()


def process_file_input(file_path: str) -> dict:
    """
    Process a single file and return structured data for LLM + RAG.
    
    Returns:
        dict with keys:
            - 'text': str - Extracted text content
            - 'image_base64': str|None - Base64 encoded image (if image file)
            - 'filename': str - Original filename
            - 'type': str - File type (pdf, image, text)
    """
    path = Path(file_path)
    if not path.exists():
        raise FileNotFoundError(f"File not found: {file_path}")

    mime_type, _ = mimetypes.guess_type(path)
    extension = path.suffix.lower()
    
    result = {
        'text': '',
        'image_base64': None,
        'filename': path.name,
        'type': 'unknown'
    }
    
    print(f"🔄 Processing: {path.name} ({extension})")

    # ====================================================
    # CASE 1: IMAGES
    # ====================================================
    if extension in ['.png', '.jpg', '.jpeg', '.webp', '.bmp', '.gif']:
        result['type'] = 'image'
        
        try:
            img = Image.open(path)
            # Convert to base64 for LLM vision
            result['image_base64'] = image_to_base64(img)
            print("   📸 Image loaded and encoded")
            
            # Try OCR if Tesseract available
            if os.path.exists(TESSERACT_PATH):
                try:
                    ocr_text = pytesseract.image_to_string(img)
                    if ocr_text.strip():
                        result['text'] = ocr_text
                        print(f"   🔤 OCR extracted: {len(ocr_text)} chars")
                except Exception as e:
                    print(f"   ⚠️ OCR failed: {e}")
            else:
                print("   ⚠️ Tesseract not found, skipping OCR")
                
        except Exception as e:
            print(f"   ❌ Image processing failed: {e}")
            raise
    
    # ====================================================
    # CASE 2: PDFs
    # ====================================================
    elif extension == '.pdf':
        result['type'] = 'pdf'
        
        try:
            reader = PdfReader(path)
            extracted_texts = []
            
            for page_num, page in enumerate(reader.pages, 1):
                page_text = page.extract_text() or ""
                extracted_texts.append(page_text)
            
            result['text'] = "\n\n".join(extracted_texts)
            print(f"   📄 PDF extracted: {len(reader.pages)} pages, {len(result['text'])} chars")
            
        except Exception as e:
            print(f"   ❌ PDF processing failed: {e}")
            raise
    
    # ====================================================
    # CASE 3: TEXT FILES
    # ====================================================
    elif extension in ['.txt', '.md', '.csv', '.json']:
        result['type'] = 'text'
        
        try:
            with open(path, 'r', encoding='utf-8') as f:
                result['text'] = f.read()
            print(f"   📝 Text file loaded: {len(result['text'])} chars")
            
        except Exception as e:
            print(f"   ❌ Text file processing failed: {e}")
            raise
    
    else:
        print(f"   ⚠️ Unsupported file type: {extension}")
        result['text'] = f"[Unsupported file type: {extension}]"
    
    return result


def batch_process_files(file_paths: list[str]) -> list[dict]:
    """Process multiple files and return list of processed data."""
    results = []
    
    for file_path in file_paths:
        try:
            processed = process_file_input(file_path)
            results.append(processed)
        except Exception as e:
            print(f"❌ Failed to process {file_path}: {e}")
            # Continue with other files
            continue
    
    return results
